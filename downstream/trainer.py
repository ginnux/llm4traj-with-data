...
class Trainer:
    ...
    def train(self):
        ...
        with trange(self.num_epoch, desc=desc_text % 0.0) as tbar:
            for epoch_i in tbar:
                ...
                self.run.log({'val_epoch': epoch_i ,'val_metric': val_metric, 'train_loss': train_loss})
    ...
    def train_epoch(self, epoch_i):
        ...
        for i, batch_meta in enumerate(tbar):
            ...
            if self.run is not None:
                self.run.log({"epoch":epoch_i, 'val_iter': epoch_i * self.num_iter + i, 'val_iter_loss':mean_loss, 'val_iter_single_loss':loss_log[-1]})
    ...
    def metric_and_save(self, labels, pres, save_name):
        ...
        print(metric)
        self.run.log({"final_metric":str(metric)})
        self.run.finish()
